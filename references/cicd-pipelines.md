...
```yaml
# .github/workflows/terragrunt-aws.yml
name: Terragrunt AWS
      ...
      - name: Cache Providers
        ...
        with:
          path: |
            /tmp/provider-cache
            /tmp/module-cache
          key: tg-staging-${{ hashFiles('non-prod/us-east-1/staging/**/*.hcl') }}
          restore-keys: tg-staging-

      - name: Format Check
        run: terragrunt hclfmt --terragrunt-check

      - name: Plan
        working-directory: non-prod/us-east-1/staging
        run: |
          terragrunt run-all plan \
            -out="tfplan" \
            --terragrunt-non-interactive \
            --terragrunt-parallelism ${{ env.TG_PARALLELISM }} \
            --provider-cache
      ...
      - name: Cache Providers
        ...
        with:
          path: |
            /tmp/provider-cache
            /tmp/module-cache
          key: tg-prod-${{ hashFiles('prod/us-east-1/prod/**/*.hcl') }}
          restore-keys: tg-prod-
...
```yaml
# .github/workflows/terragrunt-gcp.yml
name: Terragrunt GCP
...
jobs:
  detect-changes:
    ...
    runs-on: ubuntu-latest
    outputs:
      gcp-dev-iam: ${{ steps.filter.outputs.gcp-dev-iam }}
      gcp-dev-kms: ${{ steps.filter.outputs.gcp-dev-kms }}
      gcp-dev-storage: ${{ steps.filter.outputs.gcp-dev-storage }}
      gcp-dev-api: ${{ steps.filter.outputs.gcp-dev-api }}
    steps:
      - uses: actions/checkout@v4
      - uses: dorny/paths-filter@v3
        id: filter
  ...
  # ---------------------------------------------------------------------------
  gcp-dev-iam-plan:
    name: Plan (GCP Dev - IAM)
    runs-on: ubuntu-latest
    needs: [detect-changes]
    if: needs.detect-changes.outputs.gcp-dev-iam == 'true'
    steps:
      - uses: actions/checkout@v4
      ...
      - name: Cache Providers
        ...
        with:
          path: |
            /tmp/provider-cache
            /tmp/module-cache
          key: tg-gcp-dev-iam-${{ hashFiles('gcp-dev/us-east4/iam/**/*.hcl') }}
          restore-keys: tg-gcp-dev-iam-

      - name: Plan
        working-directory: gcp-dev/us-east4/iam
        run: |
          terragrunt run-all plan \
            -out="tfplan" \
            --terragrunt-non-interactive \
  ...
  gcp-dev-iam-apply:
    name: Apply (GCP Dev - IAM)
    runs-on: ubuntu-latest
    needs: [gcp-dev-iam-plan]
    if: github.ref == 'refs/heads/main'
    environment: gcp-dev
    steps:
      - uses: actions/checkout@v4
  ...
  # Repeat similar jobs for kms, storage, api...
```
